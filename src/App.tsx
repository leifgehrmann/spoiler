// Disabled a11y rule because this is a small app, and we have
// very fine control over the order in which buttons appear
// on the page.
/* eslint-disable jsx-a11y/tabindex-no-positive */
// We're not going to implement dialogs ourselves, so the no-alert
// rule has intentionally been disabled.
...
function App() {
  ...
  const updateTextMessage: React.ChangeEventHandler<HTMLTextAreaElement> = (element) => {
    setTextMessage(element.target.value);
    setBase64Message(encode(element.target.value));
  };
  const updateBase64Message: React.ChangeEventHandler<HTMLTextAreaElement> = (element) => {
    setBase64Message(element.target.value);
    ...
    const decodedMessages = element.target.value
      .split(/\s/)
      .map((message) => {
        try {
    ...
    setTextMessage(decodedMessages);
  };
  const clearBase64Message: React.MouseEventHandler<HTMLButtonElement> = () => {
    setTextMessage('');
    setBase64Message('');
    focusOnInput(base64InputRef);
  };
  const clearTextMessage: React.MouseEventHandler<HTMLButtonElement> = () => {
    setTextMessage('');
    setBase64Message('');
    focusOnInput(textInputRef);
  };
  const copyBase64MessageToClipboard: React.MouseEventHandler<HTMLButtonElement> = () => {
    try {
  ...
  const clearBase64InputCssHack = {
    top: 'calc(var(--app-height) / 2)',
  };
  const textSectionCssHack = {
    height: 'calc(var(--app-height) / 2)',
  };
  const bottomButtonsCssHack = {
      ...
      <div className="absolute w-full h-full">
        <div className="relative h-full">
          ...
          <div className="absolute left-0 m-4" style={bottomButtonsCssHack}>
            <InfoButton
              onClick={() => {
                alert('Encode and decode messages to share spoilers with friends!\n\n'
